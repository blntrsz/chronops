...
import { createEvidence, evidenceReactiveKeys, listEvidence } from "@/features/evidence/_atom";
import { listControls } from "@/features/control/_atom";
import { cn } from "@/lib/utils";
import { Control, Evidence } from "@chronops/domain";
import { Result, useAtomRefresh, useAtomSet, useAtomValue } from "@effect-atom/atom-react";
import { Schema } from "effect";
import React from "react";

const CreateEvidenceSchema = Evidence.CreateEvidence;

type CreateEvidenceInput = Schema.Schema.Type<typeof CreateEvidenceSchema>;

const listRefreshAtom = listEvidence(1);
const sourceOptions = ["upload", "link"] as const;

function CreateEvidenceForm() {
  const setActiveDialog = useSetActiveDialog();
  const mutate = useAtomSet(createEvidence(), { mode: "promise" });
  ...
  const [values, setValues] = React.useState<CreateEvidenceInput>({
    title: "",
    description: null,
    controlId: null,
    sourceType: "upload",
  ...
  async function onSubmit(e: React.SubmitEvent) {
    ...
    const validation = Schema.validateEither(CreateEvidenceSchema)(values);
    ...
    try {
      await mutate({
        ...
        reactivityKeys: evidenceReactiveKeys.all,
      });
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="start">
                  <DropdownMenuRadioGroup
                    ...
                    onValueChange={(value) =>
                      setValues((v) => ({ ...v, sourceType: value as Evidence.EvidenceSourceType }))
                    }
                  >
...
export function CreateEvidence({
  ...
  const open = activeDialog === "createEvidence";

  const triggerNode = trigger || <Button type="button">Create evidence</Button>;

  return (
    <div className={cn("flex", className)} {...props}>
      <ResponsiveDialog
        open={open}
        onOpenChange={(isOpen) => setActiveDialog(isOpen ? "createEvidence" : null)}
      >
