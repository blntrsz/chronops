import { GhostInput, GhostTextArea } from "@/components/ghost-input";
import { Button } from "@/components/ui/button";
import { FieldDescription } from "@/components/ui/field";
import { Skeleton } from "@/components/ui/skeleton";
import { Sidebar, SidebarContent, SidebarHeader, SidebarSeparator } from "@/components/ui/sidebar";
import { cn } from "@/lib/utils";
import { getAuditById, listAudits, updateAudit } from "@/features/audit/_atom";
import { CommentsSection } from "@/features/comment/comments-section";
import { useAutosaveFields } from "@/hooks/use-autosave-fields";
import { OrgListLayout } from "@/widgets/layout/org-list-layout";
import { useAppHeaderSlots } from "@/widgets/header/app-header-slots";
import { Result, useAtomRefresh, useAtomSet, useAtomValue } from "@effect-atom/atom-react";
import { createFileRoute } from "@tanstack/react-router";
import { DateTime } from "effect";
import { PanelRight } from "lucide-react";
import React from "react";

const updatedAtFormat = new Intl.DateTimeFormat(undefined, {
  dateStyle: "medium",
  timeStyle: "short",
});

function formatUpdatedAt(value: unknown) {
  if (value == null) return "—";
  try {
    const dt = DateTime.isDateTime(value) ? value : DateTime.unsafeMake(value as never);
    return DateTime.formatIntl(dt, updatedAtFormat);
  } catch {
    return String(value);
  }
}

function AuditSkeleton() {
  return (
    <div className="flex flex-col gap-6">
      <div className="space-y-2">
        <Skeleton className="h-7 w-1/2" />
        <Skeleton className="h-4 w-3/4" />
      </div>

      <div className="grid gap-6 lg:grid-cols-[2fr_1fr]">
        <Skeleton className="h-40 w-full" />
        <Skeleton className="h-64 w-full" />
      </div>
    </div>
  );
}

type AuditModel = {
  id: string;
  name: string;
  description?: string | null;
  scope?: string | null;
  status?: string | null;
  assessmentMethodId?: string | null;
  updatedAt?: unknown;
};

type AuditHeaderRightProps = {
  statusLabel: string;
  isMetaOpen: boolean;
  onToggle: () => void;
};

function AuditHeaderRight({ statusLabel, isMetaOpen, onToggle }: AuditHeaderRightProps) {
  return (
    <div className="flex items-center gap-2">
      <span className="text-xs text-muted-foreground">{statusLabel}</span>
      <Button
        type="button"
        variant="ghost"
        size="icon"
        onClick={onToggle}
        aria-expanded={isMetaOpen}
        aria-controls="audit-metadata"
        aria-label={isMetaOpen ? "Hide metadata" : "Show metadata"}
      >
        <PanelRight />
      </Button>
    </div>
  );
}

type AuditMainContentProps = {
  id: string;
  title: string;
  description: string;
  scope: string;
  onTitleChange: (value: string) => void;
  onDescriptionChange: (value: string) => void;
  onScopeChange: (value: string) => void;
};

function AuditMainContent({
  id,
  title,
  description,
  scope,
  onTitleChange,
  onDescriptionChange,
  onScopeChange,
}: AuditMainContentProps) {
  return (
    <div className="flex flex-col gap-6 py-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <GhostInput
          value={title}
          onChange={(e) => onTitleChange(e.target.value)}
          className="h-auto w-full min-w-0 p-0 text-2xl font-semibold focus-visible:ring-0"
        />
        <GhostTextArea
          value={description}
          onChange={(e) => onDescriptionChange(e.target.value)}
          className="text-muted-foreground min-h-12 w-full min-w-0 p-0 text-sm"
          placeholder="Description"
          rows={1}
        />
        <GhostTextArea
          value={scope}
          onChange={(e) => onScopeChange(e.target.value)}
          className="text-muted-foreground min-h-12 w-full min-w-0 p-0 text-sm"
          placeholder="Scope"
          rows={1}
        />
      </div>

      <div className="space-y-3">
        <h2 className="text-lg font-semibold">Overview</h2>
        <p className="text-muted-foreground text-sm">
          Audit detail placeholder; add sections next.
        </p>
      </div>

      <CommentsSection entityId={id as never} />
    </div>
  );
}

type AuditMetadataPanelProps = {
  audit: AuditModel;
};

function AuditMetadataPanel({ audit }: AuditMetadataPanelProps) {
  return (
    <div className="space-y-4 text-sm">
      <div>
        <div className="text-xs uppercase text-muted-foreground">ID</div>
        <div className="mt-1 font-medium text-foreground">{audit.id}</div>
      </div>
      <div>
        <div className="text-xs uppercase text-muted-foreground">Status</div>
        <div className="mt-1 font-medium text-foreground">{audit.status ?? "—"}</div>
      </div>
      <div>
        <div className="text-xs uppercase text-muted-foreground">Assessment</div>
        <div className="mt-1 font-medium text-foreground">{audit.assessmentMethodId ?? "—"}</div>
      </div>
      <div>
        <div className="text-xs uppercase text-muted-foreground">Updated</div>
        <div className="mt-1 font-medium text-foreground">{formatUpdatedAt(audit.updatedAt)}</div>
      </div>
    </div>
  );
}

export const Route = createFileRoute("/org/$slug/audit/$id")({
  component: RouteComponent,
});

function RouteComponent() {
  const { id } = Route.useParams();
  const auditState = useAtomValue(getAuditById(id as never));
  const mutate = useAtomSet(updateAudit(), { mode: "promise" });
  const refreshDetail = useAtomRefresh(getAuditById(id as never));
  const refreshList = useAtomRefresh(listAudits(1));

  const auditModel = Result.getOrElse(auditState, () => null);
  const [isMetaOpen, setIsMetaOpen] = React.useState(true);
  const [scopeValue, setScopeValue] = React.useState("");

  React.useEffect(() => {
    if (!auditModel) return;
    setScopeValue(auditModel.scope ?? "");
  }, [auditModel?.id, auditModel?.scope]);

  const { name, setName, description, setDescription, statusLabel } = useAutosaveFields({
    id: auditModel?.id ?? null,
    name: auditModel?.name,
    description: auditModel?.description,
    onSave: async ({ name: nextName, description: nextDescription }) => {
      await mutate({
        payload: {
          id: id as never,
          data: {
            name: nextName,
            description: nextDescription,
            scope: scopeValue === "" ? null : scopeValue,
          },
        },
      });
    },
    onSaved: () => {
      refreshDetail();
      refreshList();
    },
  });

  React.useEffect(() => {
    if (!auditModel) return;
    if (scopeValue === (auditModel.scope ?? "")) return;
    const timer = window.setTimeout(async () => {
      try {
        await mutate({
          payload: {
            id: id as never,
            data: {
              scope: scopeValue === "" ? null : scopeValue,
            },
          },
        });
        refreshDetail();
        refreshList();
      } catch {
        // ignore
      }
    }, 900);

    return () => window.clearTimeout(timer);
  }, [auditModel, scopeValue, id, mutate, refreshDetail, refreshList]);

  const headerRight = React.useMemo(
    () => (
      <AuditHeaderRight
        statusLabel={statusLabel}
        isMetaOpen={isMetaOpen}
        onToggle={() => setIsMetaOpen((value) => !value)}
      />
    ),
    [isMetaOpen, statusLabel],
  );

  useAppHeaderSlots({ right: headerRight }, [headerRight]);

  const audit = auditModel as AuditModel | null;

  if (auditState._tag === "Initial") {
    return <AuditSkeleton />;
  }

  if (Result.isFailure(auditState)) {
    return <FieldDescription>Failed loading audit</FieldDescription>;
  }

  if (!auditModel || !audit) {
    return <FieldDescription>Audit not found</FieldDescription>;
  }

  const mainContent = (
    <AuditMainContent
      id={id}
      title={name}
      description={description}
      scope={scopeValue}
      onTitleChange={setName}
      onDescriptionChange={setDescription}
      onScopeChange={setScopeValue}
    />
  );

  return (
    <OrgListLayout title={null} action={null} className="gap-0 pr-0">
      <div className="relative flex min-h-[calc(100vh-140px)] flex-col gap-6 lg:flex-row lg:items-stretch lg:gap-0">
        <div className="min-w-0 flex-1">{mainContent}</div>
        <Sidebar
          id="audit-metadata"
          side="right"
          collapsible="none"
          className={cn(
            "border-l transition-[width,opacity] duration-300 lg:-mt-4 lg:sticky lg:top-14 lg:h-[calc(100vh-56px)] lg:ml-4",
            isMetaOpen ? "w-full opacity-100 lg:w-[320px]" : "w-0 opacity-0",
          )}
          aria-hidden={!isMetaOpen}
        >
          <div
            className={cn("flex h-full w-full flex-col", isMetaOpen ? "" : "pointer-events-none")}
          >
            <SidebarHeader className="px-4 py-3">
              <div className="text-xs uppercase text-muted-foreground">Metadata</div>
            </SidebarHeader>
            <SidebarSeparator />
            <SidebarContent className="gap-4 px-4 py-4">
              <AuditMetadataPanel audit={audit} />
            </SidebarContent>
          </div>
        </Sidebar>
      </div>
    </OrgListLayout>
  );
}
